"""
CSV 파일을 PaddleOCR-VL 파인튜닝용 JSON 데이터셋으로 변환
"""
...
def csv_to_paddle_dataset(
    ...
    """
    CSV 파일을 PaddleOCR-VL 파인튜닝용 데이터셋으로 변환
    ...
    # PaddleOCR-VL 형식으로 변환 (OCR: 프롬프트 사용)
    train_paddle = []
    for item in train_data:
        train_paddle.append({
            "image": item["image_path"],
            "prefix": "OCR:",  # PaddleOCR-VL은 "OCR:" 프롬프트 사용
    ...
    with open(train_file, 'w', encoding='utf-8') as f:
        json.dump(train_paddle, f, ensure_ascii=False, indent=2)
    
    with open(val_file, 'w', encoding='utf-8') as f:
        json.dump(val_paddle, f, ensure_ascii=False, indent=2)
    
    print(f"\n✅ 데이터셋 생성 완료!")
    print(f"   학습 데이터: {len(train_paddle)}개 → {train_file}")
    print(f"   검증 데이터: {len(val_paddle)}개 → {val_file}")
    
    # 샘플 출력
    print(f"\n📝 샘플 데이터:")
    for i, sample in enumerate(train_paddle[:3]):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CSV를 PaddleOCR-VL 데이터셋으로 변환")
